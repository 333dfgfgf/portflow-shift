...
import {
  ArrowLeftRight, Bell, ChartNoAxesCombined, Check, ChevronRight, Clock3, Container,
...
export default function BlueSync() {
  ...
  const [manualSwapOpen,setManualSwapOpen]=useState(false);
  const me = trucks[0];
  ...
  const manualSwap = (firstId:string,secondId:string) => {
    const first = trucks.find(truck=>truck.id===firstId);
    const second = trucks.find(truck=>truck.id===secondId);
    if (!first || !second || first.id===second.id) return;
    setTrucks(swapReservations(trucks,first.id,second.id));
    setManualSwapOpen(false);
    setNotice(`${first.truckNumber}와 ${second.truckNumber}의 예약 시간을 교환했습니다.`);
  ...
  return <div className="app-shell">
    ...
    <div className="content">
      ...
      <main className="main-content">
        ...
        {view === "fleet" && <FleetScreen trucks={trucks} onManualSwap={role==="DISPATCHER"?()=>setManualSwapOpen(true):undefined}/>}
    ...
    {guideOpen && <GuideModal step={guideStep} onStep={setGuideStep} onClose={closeGuide}/>}
    {manualSwapOpen && <ManualSwapModal trucks={trucks} onClose={()=>setManualSwapOpen(false)} onSwap={manualSwap}/>}
  </div>;
}
...
function ManualSwapModal({trucks,onClose,onSwap}:{trucks:Truck[];onClose:()=>void;onSwap:(firstId:string,secondId:string)=>void}) {
  const [firstId,setFirstId]=useState(trucks[0]?.id??"");
  const [secondId,setSecondId]=useState(trucks[1]?.id??"");
  const first=trucks.find(truck=>truck.id===firstId);
  const second=trucks.find(truck=>truck.id===secondId);
  const changeFirst=(id:string)=>{setFirstId(id);if(id===secondId)setSecondId(trucks.find(truck=>truck.id!==id)?.id??"")};
  return <div className="manual-swap-wrap" role="dialog" aria-modal="true" aria-labelledby="manual-swap-title">
    <section className="manual-swap-modal">
      <div className="manual-swap-head"><div><small>DISPATCHER CONTROL</small><h2 id="manual-swap-title">수동 예약 교환</h2><p>교환할 차량 두 대를 직접 선택하세요.</p></div><button onClick={onClose} aria-label="수동 예약 교환 닫기"><X size={20}/></button></div>
      <div className="manual-select-grid">
        <label>첫 번째 차량<select value={firstId} onChange={event=>changeFirst(event.target.value)}>{trucks.map(truck=><option key={truck.id} value={truck.id}>{truck.truckNumber} · {maskPersonName(truck.driverName)} · {truck.reservationTime}</option>)}</select></label>
        <span><ArrowLeftRight size={20}/></span>
        <label>두 번째 차량<select value={secondId} onChange={event=>setSecondId(event.target.value)}>{trucks.filter(truck=>truck.id!==firstId).map(truck=><option key={truck.id} value={truck.id}>{truck.truckNumber} · {maskPersonName(truck.driverName)} · {truck.reservationTime}</option>)}</select></label>
      </div>
      {first&&second&&<div className="manual-preview">
        <article><small>{first.truckNumber}</small><strong>{first.reservationTime}</strong><span>변경 후 {second.reservationTime}</span></article>
        <ArrowLeftRight size={24}/>
        <article><small>{second.truckNumber}</small><strong>{second.reservationTime}</strong><span>변경 후 {first.reservationTime}</span></article>
      </div>}
      <div className="manual-warning"><Clock3 size={18}/><p><b>예약 시간만 서로 교환됩니다.</b><br/>차량·기사·터미널 정보는 변경되지 않으며 교환 이력은 현재 기기에 저장됩니다.</p></div>
      <div className="manual-swap-actions"><button onClick={onClose}>취소</button><button onClick={()=>onSwap(firstId,secondId)} disabled={!first||!second||firstId===secondId}>예약 시간 교환 확정</button></div>
    </section>
  </div>;
}
