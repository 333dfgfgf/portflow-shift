...
export function swapReservations(trucks:Truck[],a:string,b:string):Truck[]{const one=trucks.find(t=>t.id===a),two=trucks.find(t=>t.id===b);if(!one||!two)return trucks;return trucks.map(t=>t.id===a?{...t,reservationTime:two.reservationTime,status:"SWAP_COMPLETED" as const,points:t.points+150}:t.id===b?{...t,reservationTime:one.reservationTime,status:"SWAP_COMPLETED" as const,points:t.points+100}:t)}
